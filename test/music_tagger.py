#!/usr/bin/env python3
"""
音乐AI标签生成系统 - 优化版 (关闭思考模式, 打印Prompt)
"""
...
class MusicLabelingSystemFixed:
    def __init__(self):
        ...
        self.model_name = "qwen3.5:9b" # 修改为正确的模型名称
    ...
    def start_ollama_model(self):
        ...
        try:
            ...
            # 尝试拉取模型
            subprocess.run(['ollama', 'pull', self.model_name], capture_output=True, text=True)
            # 运行模型
    ...
    def generate_labels_with_ai(self, song_info: Dict, retry_count: int = 0) -> Optional[str]:
        ...
        try:
            ...
            # --- 修改点：打印出生成的提示词 ---
            logger.info(f"发送给AI的提示词 (Prompt):\n{prompt}")
            ...
            payload = {
                ...
                "stream": False,
                "options": {
                    # --- 修改点：关闭思考模式 (Temperature设为0) ---
                    "temperature": 0, # 原值为 0.3
                    "top_p": 0.9,
                    "max_tokens": 100
                }
            }
...
标签："""
    ...
    def get_unprocessed_songs(self, batch_size: int = 10) -> List[Dict]:
        ...
        try:
            ...
            FROM music 
            WHERE ( 
                label IS NULL OR label = '' OR label NOT LIKE '%%,%%,%%' 
                OR LENGTH(label) - LENGTH(REPLACE(label, ',', '')) < 2 
            ) 
