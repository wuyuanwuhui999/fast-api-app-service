...
> 服务名：social-service | 端口：4003 | 路径前缀：/service/social
...
- 入参（Query）：`relationId`、`type`、`pageNum`（默认 1）、`pageSize`（默认 10）
...
- 入参：`X-User-Id`（Header）+ Body（InsertCommentSchema：`content`、`parentId`、`topId`、`relationId`、`type`）
- 出参：ResultEntity
...
- 入参：`X-User-Id`（Header）+ Body（LikeRequestSchema：`relationId`、`type`）
- 出参：ResultEntity
...
|------|------|------|
| content | str | 评论内容 |
| parentId | int | 父评论 ID（一级评论传 0） |
| topId | int | 顶级评论 ID（一级评论传 0） |
| relationId | int | 关联资源 ID |
| type | str | 资源类型 |
