...
> 服务名：circle-service | 端口：4004 | 路径前缀：/service/circle
...
HTTP 接口需 token（网关注入 `X-User-Id`）；WebSocket 通过 `?token=` 参数。
...
- 出参：ResultEntity，data 为列表（含 circleLikes、circleComments，字段已转驼峰），`total` 为总数
...
- 作用：发布文字/图片朋友圈（图片 base64 保存到磁盘），并广播 WebSocket 消息"有一条新消息"
- 入参：`X-User-Id`（Header）+ Body（InsertCircleSchema：`content`、`imgs`、`relationId`、`type`、`permission`）
...
**InsertCircleSchema（发布朋友圈）**

| 字段 | 类型 | 说明 |
|------|------|------|
| content | str | 朋友圈内容 |
| imgs | str | 图片（base64，多张逗号分隔） |
| relationId | int | 关联音乐 audio_id 或电影 movie_id |
| type | str | 类型（MUSIC/MOVIE） |
