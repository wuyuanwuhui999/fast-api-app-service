...
> 服务名：movie-service | 端口：4001 | 路径前缀：/service/movie
...
分类/搜索/详情类接口无需 `X-User-Id`；用户相关接口（记录、收藏、历史）需 token（网关注入 `X-User-Id`）。下表"鉴权"列标"需"表示该接口读取 `X-User-Id`。
...
- 入参（Query）：`classify`、`category`、`label`、`star`、`director`、`keyword`（均可选）、`pageNum`（默认 1）、`pageSize`（默认 20）
